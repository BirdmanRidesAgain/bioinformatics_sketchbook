...
# Get table of derivatives
  ...
  derivs <- tibble(ln_1st_deriv = NA, ln_2nd_deriv = NA, delta_K = NA) # We manually assign 'NAs' for K=1
...
##############################################
### C_BIND derivs AND mean_loglikes ###
##############################################
mean_loglikes <- bind_cols(mean_loglikes, derivs)  
